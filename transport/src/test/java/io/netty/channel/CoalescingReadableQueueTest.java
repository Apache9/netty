...
public class CoalescingReadableQueueTest {
    ...
    public void setup() throws IOException {
        ...
        File mouseFile = File.createTempFile("netty-", ".tmp");
        mouseFile.deleteOnExit();
        FileOutputStream out = new FileOutputStream(mouseFile);
        try {
            out.write("mouse".getBytes(CharsetUtil.US_ASCII));
        } finally {
            out.close();
        }
        mouse = new DefaultFileRegion(mouseFile, 0, mouseFile.length());
    }
